...
class doc3dwcLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augmentations:  # this is for training, default false for validation\
            ...
            tex = cv2.imread(os.path.join(
                self.root[:-7], txpth)).astype(np.uint8)
    ...
    def transform(self, img, lbl):
        ...
        # normalize label
        msk = ((lbl[:, :, 0] != 0) & (lbl[:, :, 1] != 0)
               & (lbl[:, :, 2] != 0)).astype(np.uint8)*255
        xmx, xmn, ymx, ymn, zmx, zmn = 1.2539363, -1.2442188, 1.2396319, - \
            1.2289206, 0.6436657, -0.67492497   # calculate from all the wcs
